...
One IMO very underrated thing I strongly recommend doing is having a file that logs every checkpoint generating during a project. The idea here is that if I do some training run early on in a project and don't copy the checkpoint path (e.g. on Tinker) somewhere, if I want to find it again very late in the project (e.g. to do some last-minute evaluation), I should be able to have a centralized place to search for and find it. This could be pretty simple: a .md file with a one-sentence description of each checkpoint for a mini project, or a JSON file with info about the checkpoints—such as the path itself, paths to relevant things relating to it such as results logs, a one-sentence description of what kind of run it was from, what model it was, path (or equivalent) to the dataset, etc—you can choose as is appropriate (consulting me if you want).
